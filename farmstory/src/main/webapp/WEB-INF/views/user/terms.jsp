<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ taglib prefix="c" uri="jakarta.tags.core" %>
<c:set var="path" value="${pageContext.request.contextPath}" />
<!doctype html>
<html lang="en">
    <head>
    	<script src="${path}/js/terms.js"></script>
        <meta charset="UTF-8" />
        <title>팜스토리::약관</title>
        <link rel="stylesheet" href="${path}/css/style_terms.css" />
    </head>
    <body>
        <div id="container">
            <%@ include file="/WEB-INF/views/inc/_header.jsp" %>
            <div id="user">
                <section class="terms">
                    <h2 class="tit">사이트 이용약관</h2>
                    <table border="1">
                        <tr>
                            <td>
                                <textarea name="terms">${terms.basic }</textarea>
                                <label><input type="checkbox" class="terms" />&nbsp;동의합니다.</label>
                            </td>
                        </tr>
                    </table>
                    <h2 class="tit">개인정보 취급방침</h2>
                    <table border="1">
                        <tr>
                            <td>
                                <textarea name="privacy">${terms.privacy }</textarea>
                                <label><input type="checkbox" class="privacy" />&nbsp;동의합니다.</label>
                            </td>
                        </tr>
                    </table>
                    <div>
                        <a href="${path}/user/login.do" class="btn btnCancel">취소</a>
                        <a href="#" onclick="checkTerms()" class="btn btnNext">다음</a>
                    </div>
                </section>
            </div>
            <%@ include file="/WEB-INF/views/inc/_footer.jsp" %>
        </div>
    </body>
</html>